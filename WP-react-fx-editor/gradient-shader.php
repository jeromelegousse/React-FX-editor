...
function gs_ensure_view_script_enqueued() {
  ...
  if (!wp_script_is($handle, 'registered')) {
    wp_register_script(
      ...
      [],
      filemtime(__DIR__ . '/assets/frontend.js'),
      true
    );
  }

  if (!$localized) {
    wp_localize_script($handle, 'GS_CONFIG', gs_get_config_payload());
    $localized = true;
  }
...
add_filter('render_block', function ($content, $block) {
  if (is_array($block) && ($block['blockName'] ?? '') === 'gs/gradient-shader') {
    gs_ensure_view_script_enqueued();
  }
  return $content;
}, 10, 2);
